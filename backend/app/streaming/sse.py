    ...
    if is_coding_request:
        system_msg = {
            ...
            "content": (
                ...
                "ВЫПОЛНЕНИЕ КОМАНД (ТЕРМИНАЛ): Ты имеешь возможность запускать команды в терминале на локальном устройстве пользователя. Если пользователю требуется запустить команду, установить библиотеку (через pip, npm, flutter pub и т.д.), скомпилировать или собрать что-то (например, flutter build apk), запустить тест или скрипт, выведи специальный текстовый блок прямо в своем ответе в таком формате:\n"
                "<execute_command>команда_терминала</execute_command>\n"
                "Пользователь увидит эту команду и сможет запустить её. Выполняй команды последовательно, по одной за раз, если требуется несколько шагов."
            )
        }
